...
export default function FranchiseMap() {
    ...
    const fetchLocations = async (company = null) => {
        ...
        try {
            ...
            const msg =
                err?.response?.data?.message || err.message || "Unable to load franchise locations";
            setError(msg);
            setLocations([]);
        } finally {
            setLoading(false);
    ...
    const computeCenter = (data) => {
        const items = (data && data.length) ? data : (locations && locations.length ? locations : []);
        const latitudes = items.map(d => Number(d.lat)).filter(Boolean);
        const longitudes = items.map(d => Number(d.lng)).filter(Boolean);
        const centerLat = latitudes.length ? latitudes.reduce((s, v) => s + v, 0) / latitudes.length : 19.0760;
        const centerLng = longitudes.length ? longitudes.reduce((s, v) => s + v, 0) / longitudes.length : 72.8777;
    ...
    const fallback = [
        { id: "s1", franchiseName: "Sample Franchise 1", institutionName: "Snipe Partner", lat: 19.0760, lng: 72.8777, address: "Mumbai" },
        { id: "s2", franchiseName: "Sample Franchise 2", institutionName: "Snipe Partner", lat: 19.1190, lng: 72.8356, address: "Andheri" },
    ];

    const dataToShow = (filteredData && filteredData.length) ? filteredData : ((locations && locations.length) ? filteredData : fallback);
        ...
        <div className={styles.outer}>
            <div className={styles.headingRow}>
                ...
                <div className={styles.filterWrap}>
                    <label htmlFor="institutionSelect" className={styles.filterLabel}><h3>Institutions: </h3></label>
                    <select
            ...
            <div className={styles.container}>
                <MapContainer center={[centerLat, centerLng]} zoom={11} className={styles.map} scrollWheelZoom>
                    ...
                    {dataToShow.map((f) => {
                        ...
                        return (
                            <Marker key={f.id || `${lat}-${lng}`} position={[lat, lng]}>
                                <Tooltip direction="top" sticky>{title}</Tooltip>
                                <Popup>
                                    <div style={{ textAlign: "left", color: "black" }}>
                                        <strong>{title}</strong>
                                        {f.institutionName && <div style={{ marginTop: 4 }}>{f.institutionName}</div>}
                                        {f.address && <div style={{ marginTop: 6, opacity: 0.95 }}>{f.address}</div>}
                                    </div>
                                </Popup>
                            </Marker>
                        );
